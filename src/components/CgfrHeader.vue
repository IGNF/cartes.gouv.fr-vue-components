<script setup>
import { computed, useAttrs, onMounted } from 'vue';
import { DsfrHeader, useScheme } from '@gouvminint/vue-dsfr';
import CgfrHeaderNavigation from './CgfrHeaderNavigation.vue';
import { getHeaderDefaults } from './CgfrHeader.defaults.js';

const { theme } = useScheme();

const props = defineProps({
  baseUrl: {
    type: String,
    default: undefined,
  },
  badgeText: {
    type: String,
    default: 'Bêta',
  },
  badgeColor: {
    type: String,
    default: 'green-emeraude',
  },
  badgeIcon: {
    type: String,
    default: 'fr-icon-beta',
  },
  compact: {
    type: Boolean,
    default: false,
  },
  authenticated: {
    type: Boolean,
    default: false,
  },
  user: {
    type: Object,
    default: () => ({}),
  },
  iamDisable: {
    type: Boolean,
    default: false,
  },
});

const attrs = useAttrs();

const headerProps = computed(() => {
  const defaults = getHeaderDefaults({
    baseUrl: props.baseUrl,
    iamDisable: props.iamDisable,
  });

  return {
    ...defaults,
    ...attrs,
    // Hack pour vuedsfr
    // ajoute un faux quicklinks (pour pouvoir utiliser le slot after-quick-links)
    quickLinks: [
      {
        label: '',
        to: '/',
      },
    ],
  };
});

let operatorImgSrc = computed(() => {
  let lightOrDark = theme.value === 'light' ? '' : '-dark';
  return `https://cartes.gouv.fr/img/header/cartes-gouv-logo${lightOrDark}.svg`;
});

onMounted(() => {
  // modifie le badge beta en explorer
  let badge = document.querySelector('.fr-header__service-title .fr-badge');
  if (badge && props.badgeText) {
    badge.classList.remove('fr-badge--green-emeraude');
    badge.classList.add(`fr-badge--${props.badgeColor}`);
    badge.innerHTML = `
      <span class="${props.badgeIcon} fr-ml-n1v" style="transform:scale(0.5)"></span>
      ${props.badgeText}
    `;
  }
});
</script>

<template>
  <DsfrHeader
    show-beta
    :service-title="headerProps.serviceTitle"
    :service-description="compact ? false : headerProps.serviceDescription"
    :operator-img-src="operatorImgSrc"
    :logo-text="compact ? [] : headerProps.logoText"
    :quick-links="headerProps.quickLinks"
    :class="{ 'fr-header--compact': compact }"
  >
    <template #after-quick-links>
      <CgfrHeaderNavigation
        :items="headerProps.items"
        :authenticated="authenticated"
        :user="user"
      />
      <slot />
    </template>
  </DsfrHeader>
</template>

<style lang="scss">
@use '../assets/mq' as *;

// fixe taille du logo
.fr-header__operator img {
  min-width: 0;
  width: 55px;
  height: 60px;
}

// fix texte sur 2 lignes
.fr-header .fr-nav__list {
  margin: 0;
}

// mobile (SM/MD): le menu ne couvre pas toute la page
@include between(sm, lg) {
  .fr-header__menu {
    max-width: 360px;
    left: auto;
    right: 0;
    box-shadow: var(--raised-shadow);
  }
}

/**
 * Header mode compact
 *
 * hauteur totale = 56px
 * hauteur du contenu = 40px
 *
 * */
.fr-header--compact {
  .fr-header__body-row {
    padding: 0.5rem;
  }
  .fr-header__brand {
    padding: 0.125rem 0;
  }
  .fr-header__logo {
    padding-top: 0;
    padding-bottom: 0;

    .fr-logo {
      padding-top: 0.9rem; // vertical-align
      transform: scale(1.33);
    }
    .fr-logo::after {
      content: none; // Supprime le pseudo-élément
    }
  }
  .fr-header__operator {
    padding: 0;
    margin-left: 0.75rem;

    img {
      // conserve les pixels nets
      width: 33px;
      height: 36px;
    }
  }
  .fr-header__service {
    padding-top: 0;
    padding-bottom: 0;
    padding-right: 0.75rem;
  }
  /* mobile */
  @include max(lg) {
    .fr-header__body-row {
      padding: 0;
    }
    .fr-header__brand {
      padding: 0.625rem 0.5rem;
    }
    .fr-header__brand-top {
      width: auto;
    }
    .fr-header__navbar {
      position: absolute;
      top: 0;
      right: 8px;
    }
    .fr-header__service::before {
      content: none;
    }
  }
}

// hack vuedsfr
:is(.fr-header__menu-links, .fr-header__tools-links) > div:first-child {
  display: none !important;
}
</style>
